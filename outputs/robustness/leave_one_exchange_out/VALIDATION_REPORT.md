...
Each scenario recomputes the effective minutes, dollar-volume proxy weights, concentration measures, VWAP, LWMP, and pass-through quantities from the processed per-exchange data rather than deleting a venue from already-computed aggregates.
...
Liquidity-Weighted Median Price (LWMP) uses synchronized dollar-volume proxy weights to represent relative venue liquidity. Under the lower weighted-median convention, it selects the first ordered price whose cumulative weight reaches 0.5. If max_share > 0.5, the dominant exchange must be the LWMP pivot, so lock-in dominant-price displacement should produce LWMP pass-through close to 1.
