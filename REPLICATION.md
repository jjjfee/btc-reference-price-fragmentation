...
Raw data are not committed. Do not place credentials or Kaggle tokens in the repository.
...
```bash
python src/audit/run_btc_fixed_composition_checks.py --project-root . --seed 42
```
...
The `--headline-only` flag stops after the exact headline reproduction gate, but it still requires the external event-level inputs used by that gate.
...
python -m compileall -q src scripts archive
```

The validator checks manifest path existence, exact manuscript-facing numbers, the 29-check gate, and personal absolute-path hygiene.
